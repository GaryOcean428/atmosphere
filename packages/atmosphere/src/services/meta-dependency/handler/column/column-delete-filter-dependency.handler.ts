 ...
 *
 * Sweeps:
 *  - `atm_sorts` rows where `fk_column_id = colId`
 *  - `atm_sorts` rows where `fk_lookup_col_id = colId` (per-lookup sort config —
 ...
 *  - `atm_filter_exp` rows where `fk_column_id = colId` OR `fk_value_col_id = colId`
