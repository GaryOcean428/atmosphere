 ...
 * to blocking it by setting `ATMOSPHERE_DISABLE_DB_SSL_FILE_PATHS=true`.
 *
...
export function validateDbConnectionSslPaths(ssl: unknown): void {
  ...
  if (!isCloud && process.env.ATMOSPHERE_DISABLE_DB_SSL_FILE_PATHS !== 'true') return;
 ...
 *
 * Set `ATMOSPHERE_ALLOW_LOCAL_EXTERNAL_DBS=true` (or `ATMOSPHERE_DISABLE_SSRF_PROTECTION=true`)
...
export async function validateDbConnectionHost(host: unknown): Promise<void> {
  ...
  if (
    ...
    AtError.badRequest('Connection to internal hosts is not allowed');
  }
