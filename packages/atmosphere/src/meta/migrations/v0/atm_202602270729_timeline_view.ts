...
const up = async (knex: Knex) => {
  await knex.schema.createTable(MetaTable.TIMELINE_VIEW, (table) => {
    ...
    table.index(
      ...
      'atm_timeline_view_v2_base_id_fk_workspace_id_index',
    );
    table.index(['fk_view_id'], 'atm_timeline_view_v2_oldpk_idx');
  });
  ...
  await knex.schema.createTable(MetaTable.TIMELINE_VIEW_COLUMNS, (table) => {
    ...
    table.index(
      ['base_id', 'fk_workspace_id'],
      'atm_timeline_view_columns_v2_base_id_fk_workspace_id_index',
    );
    table.index(
      ['fk_view_id', 'fk_column_id'],
      'atm_timeline_view_columns_v2_fk_view_id_fk_column_id_index',
    );
    table.index(['id'], 'atm_timeline_view_columns_v2_oldpk_idx');
  });
  ...
  await knex.schema.createTable(MetaTable.TIMELINE_VIEW_RANGE, (table) => {
    ...
    table.index(
      ['base_id', 'fk_workspace_id'],
      'atm_timeline_view_range_v2_base_id_fk_workspace_id_index',
    );
    table.index(['id'], 'atm_timeline_view_range_v2_oldpk_idx');
  });
};
