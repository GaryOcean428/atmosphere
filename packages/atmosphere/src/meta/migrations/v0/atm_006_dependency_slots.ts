...
const up = async (knex: Knex) => {
  await knex.schema.alterTable(MetaTable.DEPENDENCY_TRACKER, (table) => {
    ...
    // queryable_field_0: Workflow=nodeType (e.g., "atmosphere.trigger.after_insert")
  ...
  await knex.schema.alterTable(MetaTable.DEPENDENCY_TRACKER, (table) => {
    if (!['mysql', 'mysql2'].includes(knex.client.config.client)) {
      table.index(
        'queryable_field_0',
        'atm_dependency_tracker_queryable_field_0_idx',
      );
      table.index(
        'queryable_field_1',
        'atm_dependency_tracker_queryable_field_1_idx',
      );
    }
  });
};
