...
// unique instead of unique per base. atm_092 and atm_013 moved every other base-scoped
// table to a composite key and these were missed — atm_permission_subjects and
// atm_rls_policy_subjects are the same shape and only the first was converted.
//
...
const LOG = '[atm_202607271818_composite_pk_missing_tables_2]';

// `old` is retained as a plain index so lookups still querying by the previous key
// stay indexed — same convention as atm_013's `_oldpk_idx`.
...
> = {
  [MetaTable.RLS_POLICY_SUBJECTS]: {
    ...
    name: 'atm_rls_policy_subjects_pk',
  },
  [MetaTable.VIEW_SECTIONS]: {
    pk: ['base_id', 'id'],
    old: ['id'],
    name: 'atm_view_sections_pkey',
  },
};
...
const DATE_DEP_UNIQUE = 'atm_date_dep_gantt_view_id_unique';
