...
const up = async (knex: Knex) => {
  ...
  await knex.schema.alterTable(MetaTableOldV2.OUTLINE_VIEW, (table) => {
    table.index(
      ...
      'atm_outline_view_v2_base_id_fk_workspace_id_index',
    );
    table.index(['fk_view_id'], 'atm_outline_view_v2_fk_view_id_index');
  });

  await knex.schema.alterTable(MetaTableOldV2.OUTLINE_VIEW_COLUMNS, (table) => {
    table.index(
      ['base_id', 'fk_workspace_id'],
      'atm_outline_view_columns_v2_base_id_fk_workspace_id_index',
    );
    table.index(
      ['fk_view_id', 'fk_column_id'],
      'atm_outline_view_columns_v2_fk_view_id_fk_column_id_index',
    );
    table.index(['fk_view_id'], 'atm_outline_view_columns_v2_fk_view_id_index');
  });

  await knex.schema.alterTable(MetaTableOldV2.OUTLINE_VIEW_LEVELS, (table) => {
    table.index(
      ['base_id', 'fk_workspace_id'],
      'atm_outline_view_levels_v2_base_id_fk_workspace_id_index',
    );
    table.index(['fk_view_id'], 'atm_outline_view_levels_v2_fk_view_id_index');
  });

  await knex.schema.alterTable(MetaTable.SORT, (table) => {
    table.string('fk_level_id', 20);
    table.index(['fk_level_id'], 'atm_sort_v2_fk_level_id_index');
  });

  await knex.schema.alterTable(MetaTable.FILTER_EXP, (table) => {
    table.string('fk_level_id', 20);
    table.index(['fk_level_id'], 'atm_filter_exp_v2_fk_level_id_index');
  });
};

const down = async (knex: Knex) => {
  await knex.schema.alterTable(MetaTable.FILTER_EXP, (table) => {
    table.dropIndex([], 'atm_filter_exp_v2_fk_level_id_index');
    table.dropColumn('fk_level_id');
  });
  await knex.schema.alterTable(MetaTable.SORT, (table) => {
    table.dropIndex([], 'atm_sort_v2_fk_level_id_index');
    table.dropColumn('fk_level_id');
  });
