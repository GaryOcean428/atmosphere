 ...
 *
 * Distinct from `atm_view_sections`, which groups views *inside* one table and is
 ...
 * among, so `order` here is comparable against top-level `atm_models_v2.order`
...
const up = async (knex: Knex) => {
  await knex.schema.createTable(MetaTable.BASE_SECTIONS, (table) => {
    ...
    table.index('id', 'atm_base_sections_id_idx');
    table.index(['base_id', 'fk_workspace_id'], 'atm_base_sections_context');
  });

  // One FK column covers every sidebar entity kind: tables, documents AND
  // dashboards are all atm_models_v2 rows (discriminated by `type`) — the
  // legacy atm_dashboards_v2 table holds no live rows.
