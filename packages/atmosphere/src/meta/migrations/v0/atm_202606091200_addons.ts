...
const up = async (knex: Knex) => {
  await knex.schema.createTable('atm_addons', (table) => {
    ...
    table.index('addon_key', 'atm_addons_addon_key_idx');
    table.index('stripe_product_id', 'atm_addons_stripe_product_idx');
  });

  await knex.schema.createTable('atm_subscription_addons', (table) => {
    table.string('id', 20).primary();
    ...
    table.text('meta');
    table.timestamps(true, true);
    table.index('fk_subscription_id', 'atm_subscription_addons_sub_idx');
    table.index('addon_key', 'atm_subscription_addons_key_idx');
  });
};

const down = async (knex: Knex) => {
  await knex.schema.dropTableIfExists('atm_subscription_addons');
  await knex.schema.dropTableIfExists('atm_addons');
};
