...
export default class UserRefreshToken {
  ...
  public static async insert(
    ...
    if (!('expires_at' in insertObj)) {
      insertObj.expires_at = dayjs()
        .add(ATMOSPHERE_REFRESH_TOKEN_EXP_IN_DAYS, 'day')
        .toDate();
    }
  ...
  static async updateOldToken(
    ...
    const affected = await ncMeta.metaUpdate(
      ...
      {
        token: newToken,
        expires_at: dayjs().add(ATMOSPHERE_REFRESH_TOKEN_EXP_IN_DAYS, 'day').toDate(),
      },
      {
        token: oldToken,
      },
  ...
  static async deleteToken(token: string, ncMeta = Atmosphere.ncMeta) {
  ...
  static async deleteAllUserToken(userId: string, ncMeta = Atmosphere.ncMeta) {
    return await ncMeta.metaDelete(
      RootScopes.ROOT,
      RootScopes.ROOT,
      MetaTable.USER_REFRESH_TOKENS,
