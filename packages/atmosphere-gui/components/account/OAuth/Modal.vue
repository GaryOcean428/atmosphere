...
const supportedDocs: SupportedDocsType[] = [
  {
    title: 'Atmosphere OAuth Client Setup',
    href: 'https://docs.atmosphere.dev/atmosphere-gui/oauth-client-setup',
  },
  {
    title: 'Atmosphere OAuth Client Setup',
    href: 'https://docs.atmosphere.dev/atmosphere-gui/oauth-client-setup',
...
<template>
  <AtModal v-model:visible="modalVisible" :show-separator="true" size="large" wrap-class-name="atm-modal-oauth-client-create-edit">
    <template #header>
      <div class="flex w-full items-center p-2 justify-between">
        <div class="flex items-center gap-3 pl-1 flex-1">
          ...
          <span class="text-atm-content-gray-emphasis truncate font-semibold text-xl"> Create OAuth Client </span>
        </div>
        ...
        <div class="flex justify-end items-center gap-3 pr-0.5 flex-1">
          ...
          <AtButton v-if="showSuccessView" type="primary" size="small" @click="handleClose"> {{ $t('general.done') }} </AtButton>
          <AtButton type="text" size="small" data-testid="atm-close-oauth-modal" @click.stop="handleClose">
          ...
          <div class="flex flex-col gap-4">
            <div class="flex flex-col gap-2">
              ...
              <div class="flex items-center gap-2">
                <a-input :value="createdClient?.client_id" readonly class="atm-input-shadow !rounded-lg flex-1" />
                <AtButton type="secondary" size="small" @click="copyToClipboard(createdClient?.client_id, 'Client ID')">
            ...
            <div v-if="createdClient?.client_secret" class="flex flex-col gap-2">
              ...
              <div class="flex items-center gap-2">
                <a-input :value="createdClient?.client_secret" readonly class="atm-input-shadow !rounded-lg flex-1" />
                <AtButton type="secondary" size="small" @click="copyToClipboard(createdClient?.client_secret, 'Client Secret')">
                  <GeneralIcon icon="copy" />
                </AtButton>
              </div>
            </div>
        ...
        <div v-else class="flex flex-col max-w-[640px] w-full mx-auto gap-3">
          <a-form :model="clientRef" name="create-oauth-client" layout="vertical" class="flex flex-col gap-6">
            ...
            <a-form-item class="!mb-0 flex-1">
              ...
              <a-radio-group v-model:value="clientRef.client_type" class="atm-input-shadow">
...
<style lang="scss">
.atm-modal-oauth-client-create-edit {
  z-index: 1050;
  a {
