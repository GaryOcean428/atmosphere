...
<template>
  ...
  >
    <AtTooltip v-if="tempFiles.length === 0" class="absolute top-3 right-3">
      <AtButton type="text" class="!border-0" size="xsmall" @click="closeMenu">
    ...
    <div v-if="tempFiles.length > 0" class="flex w-full border-b-1 py-1 h-9.5 items-center justify-between top-0">
      <AtButton type="text" size="small" @click="clearAll">
      ...
      <AtButton type="text" size="small" @click="open">
    ...
    <div
      ref="dropZoneRef"
      :class="{
        'border-atm-border-brand': isOverDropZone,
    ...
    >
      <div v-if="!tempFiles.length" class="flex cursor-pointer items-center justify-center flex-col gap-4">
        ...
        <template v-if="isOverDropZone">
          <component :is="iconMap.upload" class="w-8 text-atm-content-brand h-8" />
          <h1 class="text-atm-content-brand font-bold">{{ $t('labels.dropHere') }}</h1>
        </template>
      </div>
      <template v-else>
        <div
          class="grid overflow-y-auto flex-grow-1 atm-scrollbar-md grid-cols-4 w-full h-full items-start py-2 justify-center gap-4"
        >
          <div v-for="file in tempFiles" :key="file.name" class="flex gap-1.5 group max-w-34 pb-4 flex-col relative">
            <div
              ...
              style="height: 140px"
              class="flex items-center justify-center rounded-md bg-atm-bg-gray-dark"
            >
            ...
            <div class="relative text-[12px] font-semibold items-center text-atm-content-gray flex">
              <AtTooltip class="flex-auto truncate" placement="bottom">
              ...
              <div class="flex-none hide-ui transition-all transition-ease-in-out !h-4 flex items-center bg-atm-bg-default">
                <AtTooltip placement="bottom">
                  ...
                  <component
                    :is="iconMap.delete"
                    class="!text-atm-content-red-medium w-3 h-3 cursor-pointer"
    ...
    <div v-if="tempFiles.length" class="flex gap-2 pt-2 bg-atm-bg-default w-full items-center justify-end">
      <AtButton :disabled="isLoading" type="secondary" size="small" @click="closeMenu">
        {{ $t('labels.cancel') }}
      </AtButton>

      <AtButton :loading="isLoading" data-testid="atm-upload-file" size="small" @click="emits('upload', tempFiles)">
