...
<template>
  <GeneralModal v-model:visible="visible" class="atm-attachment-rename-modal" size="small">
      ...
      <a-form class="w-full h-full" no-style :model="form" @finish="renameFile(form.title)">
        <a-form-item class="w-full !mb-0" name="title" :rules="rules.title">
          <a-input
            ...
            class="w-full atm-input-sm atm-input-shadow"
            :placeholder="$t('general.rename')"
          />
        </a-form-item>
        <div class="flex flex-row gap-x-2 mt-2.5 pt-2.5 justify-end">
          <AtButton key="back" html-type="back" size="small" type="secondary">{{ $t('general.cancel') }}</AtButton>
          <AtButton key="submit" html-type="submit" size="small" type="primary">{{ $t('general.confirm') }}</AtButton>
        </div>
      </a-form>
    </div>
  </GeneralModal>
