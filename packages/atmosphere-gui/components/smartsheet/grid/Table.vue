...
import type { ButtonType, ColumnReqType, ColumnType, PaginatedType, TableType, ViewType } from 'atmosphere-sdk'
import { UITypes, ViewTypes, isAIPromptCol, isAutoGeneratedColumn, isLinksOrLTAR, isSystemColumn, isVirtualCol } from 'atmosphere-sdk'
...
const gridWrapperClass = computed<string>(() => {
  ...
  if (headerOnly.value !== true) {
    if (!scrollParent.value) {
      classes.push('atm-scrollbar-x-lg !overflow-auto')
    }
  } else {
...
<template>
  <div class="flex flex-col" :class="`${headerOnly !== true ? 'h-full w-full' : ''}`">
    ...
    <div ref="gridWrapper" class="atm-grid-wrapper min-h-0 flex-1 relative" :class="gridWrapperClass">
      <div
        v-show="isPaginationLoading && !headerOnly"
        class="flex items-center justify-center bg-atm-bg-default/80 absolute l-0 t-0 w-full h-full z-10 pb-10 pointer-events-none"
      >
      ...
      >
        <div>
          <table
            class="xc-row-table atm-grid backgroundColorDefault !h-auto bg-atm-bg-default sticky top-0 z-5"
            :class="{
              mobile: isMobileMode,
          ...
          >
            <thead v-show="hideHeader !== true" ref="tableHeadEl">
              ...
              <tr v-show="!isViewColumnsLoading" class="atm-grid-header">
                <th class="w-[64px] min-w-[64px]" data-testid="grid-id-column">
                  <div class="w-full h-full flex pl-2 pr-1 items-center" data-testid="atm-check-all">
                    ...
                    <template v-else>
                      <div class="text-atm-content-gray-muted">#</div>
                    </template>
                  </div>
                </th>
                <th
                ...
                >
                  ...
                  >
                    <a-dropdown
                      ...
                      :trigger="['click']"
                      :overlay-class-name="`atm-dropdown-add-column ${addColumnDropdown ? 'rounded-2xl' : ''}`"
                    ...
                    >
                      ...
                      <template v-if="appInfo.ee && persistMenu && meta?.id" #overlay>
                        <AtMenu class="predict-menu" variant="small">
                          <AtSubMenu v-if="predictedNextColumn?.length" key="predict-column" class="py-0 px-0 w-full">
                            <template #title>
                              <div class="flex flex-row items-center px-1 py-0.5 gap-1 w-full">
                                <MdiTableColumnPlusAfter class="flex h-[1rem] text-atm-content-gray-muted" />
                                <div class="text-xs flex-1">
                                  {{ $t('activity.predictColumns') }}
                                </div>
                                <MdiChevronRight class="text-atm-content-gray-muted" />
                              </div>
                            </template>
                            <template #expandIcon></template>
                            <template v-for="col in predictedNextColumn" :key="`predict-${col.title}-${col.type}`">
                              <AtMenuItem class="w-full flex items-center" @click="loadColumn(col.title, col.type)">
                                <div class="text-xs">{{ col.title }}</div>
                              </AtMenuItem>
                            </template>

                            <AtMenuItem class="flex flex-row items-center" @click="predictNextColumn(meta.id)">
                              <div class="text-atm-content-red-medium text-xs">
                                <MdiReload />
                                {{ $t('labels.generateAgain') }}
                              </div>
                            </AtMenuItem>
                          </AtSubMenu>
                          <AtMenuItem v-else class="flex flex-row items-center py-3" @click="predictNextColumn(meta.id)">
                            <!-- Predict Columns -->
                            <MdiReload v-if="predictingNextColumn" class="animate-infinite animate-spin" />
                            <MdiTableColumnPlusAfter v-else class="flex h-[1rem] text-atm-content-gray-muted" />
                            <div class="text-xs">
                              {{ $t('activity.predictColumns') }}
                            </div>
                          </AtMenuItem>
                          <AtSubMenu v-if="predictedNextFormulas?.length" key="predict-formula" class="py-0 px-0 w-full">
                            <template #title>
                              <div class="flex flex-row items-center px-1 py-0.5 gap-1 w-full">
                                <MdiCalculatorVariant class="flex h-[1rem] text-atm-content-gray-muted" />
                                <div class="text-xs flex-1">
                            ...
                            <AtMenuItem class="flex flex-row items-center" @click="predictNextFormulas(meta.id)">
                              <div class="text-atm-content-red-medium text-xs">
                                <MdiReload />
                                {{ $t('labels.generateAgain') }}
                              </div>
                          ...
                          <AtMenuItem v-else class="flex flex-row items-center py-3" @click="predictNextFormulas(meta.id)">
                            <!-- Predict Formulas -->
                            <MdiReload v-if="predictingNextFormulas" class="animate-infinite animate-spin" />
                            <MdiCalculatorVariant v-else class="flex h-[1rem] text-atm-content-gray-muted" />
                            <div class="text-xs">
                              {{ $t('activity.predictFormulas') }}
                            </div>
                          </AtMenuItem>
          ...
          <div
            v-if="!showSkeleton"
            class="table-overlay"
            :class="{ 'atm-grid-skeleton-loader': showSkeleton }"
            :style="{
            ...
            >
              <tbody v-if="headerOnly !== true" ref="tableBodyEl">
                ...
                <LazySmartsheetRow v-for="{ row, index: rowIndex } in visibleData" :key="rowIndex" :row="row">
                  <template #default="{ state }">
                    ...
                    >
                      <td
                        key="row-index"
                        class="caption atm-grid-cell w-[64px] min-w-[64px]"
                        :data-testid="`cell-Id-${rowIndex}`"
                        @contextmenu="contextMenuTarget = null"
                      >
                        <div class="w-[64px] pl-2 pr-1 items-center w-full flex gap-0.5">
                          <div
                            class="atm-row-no sm:min-w-4 text-atm-content-gray-muted flex items-center justify-between h-4 w-full"
                            :class="{
                              'toggle': !readOnly,
                          ...
                          >
                            <AtCheckbox v-model:checked="row.rowMeta.selected" />
                          </div>
                          <span class="flex-1" />

                          <div
                            class="atm-expand flex-1 flex items-center justify-end"
                            :data-testid="`atm-expand-${rowIndex}`"
                            :class="{ 'atm-comment': row.rowMeta?.commentCount }"
                          >
                            <a-spin
                            ...
                            <template v-else>
                              <span
                                ...
                                class="px-1 rounded-md rounded-bl-none transition-all border-1 border-brand-200 cursor-pointer font-sembold select-none hover:bg-brand-100 !min-h-4.5 !min-w-5 !leading-[18px] inline-block text-atm-content-brand bg-atm-bg-brand"
                                :class="{
                              ...
                              <div
                                ...
                                class="cursor-pointer flex items-center border-1 border-atm-border-gray-light active:ring rounded-md p-0.75 hover:(bg-atm-bg-default border-atm-border-gray-medium)"
                              >
                                <component
                                  :is="iconMap.maximize"
                                  v-if="expandForm"
                                  v-e="['c:row-expand:open']"
                                  class="select-none atm-row-expand opacity-90 w-3.5 h-3.5"
                                  @click="expandAndLooseFocus(row, state)"
                                />
                              </div>
                            </template>
                      ...
                      <SmartsheetTableDataCell
                        ...
                        :class="{
                          ...
                          'active-cell':
                            (activeCell.row === rowIndex && activeCell.col === 0) ||
                            (selectedRange._start?.row === rowIndex && selectedRange._start?.col === 0),
                          'atm-required-cell': cellMeta[rowIndex][0].isColumnRequiredAndNull && !isPublicView,
                      ...
                      <SmartsheetTableDataCell
                        ...
                        :class="{
                          ...
                          'active-cell':
                            (activeCell.row === rowIndex && activeCell.col === colIndex) ||
                            (selectedRange._start?.row === rowIndex && selectedRange._start?.col === colIndex),
                          'atm-required-cell': cellMeta[rowIndex][colIndex].isColumnRequiredAndNull && !isPublicView,
                          'align-middle': !rowHeightEnum || rowHeightEnum === 1,
                          'align-top': rowHeightEnum && rowHeightEnum !== 1,
                ...
                <tr
                  ...
                  class="text-left atm-grid-add-new-cell cursor-pointer group relative z-3 xs:hidden"
                  :class="{
                ...
                >
                  ...
                  <td class="!border-atm-border-gray-light" :colspan="visibleColLength"></td>
                </tr>
              </tbody>
            </table>
        ...
        <template #overlay>
          <AtMenu class="!rounded !py-0" variant="small" @click="contextMenu = false">
            ...
            <template v-if="hasEditPermission && !isDataReadOnly">
              <AtDivider v-if="!(!contextMenuClosing && !contextMenuTarget && data.some((r) => r.rowMeta.selected))" />
              <AtMenuItem
    ...
    <div class="relative">
      <LazySmartsheetPagination
        ...
        :key="`atm-pagination-${isMobileMode}`"
      ...
      >
        <template v-if="isAddingEmptyRowAllowed && !showSkeleton" #add-record>
          <div class="flex ml-1">
            ...
            >
              <div
                data-testid="atm-pagination-add-record"
                class="flex items-center px-2 text-atm-content-gray-subtle2 hover:text-atm-content-gray-extreme"
              >
                <span>
              ...
              <template #icon>
                <component :is="iconMap.arrowUp" class="text-atm-content-gray-subtle2 h-4 w-4" />
              </template>
            </a-dropdown-button>
          </div>
        </template>
    ...
    <div
      ...
      @click.stop
    >
      <AtDropdown v-if="isAddingEmptyRowAllowed && !showSkeleton">
        ...
        <template #overlay>
          <AtMenu variant="small">
            <AtMenuItem v-e="['c:row:add:grid']" class="atm-new-record-with-grid group" @click="onNewRecordToGridClick">
...
.atm-grid-pagination-wrapper .ant-dropdown-button {
  > .ant-btn {
    @apply !p-0 !rounded-l-lg hover:border-atm-border-gray-extradark;
  }
...
.atm-grid-wrapper {
  ...
  tbody td:not(.atm-grid-add-new-cell-item):nth-child(1) {
    position: sticky !important;
    left: 0;
    z-index: 4;
  ...
  .desktop {
    ...
    tbody tr:not(.atm-grid-add-new-cell) td:nth-child(2) {
      position: sticky !important;
      z-index: 4;
      left: 64px;
      // background: white;
  ...
  .atm-grid-skeleton-loader {
    ...
    tbody td:not(.atm-grid-add-new-cell-item):nth-child(2) {
      @apply border-r-1 !border-r-atm-border-gray-extralight;
    }
  }
}
...
.atm-grid-row {
  ...
  &.selected-row {
    td.atm-grid-cell:not(.active),
    td:nth-child(2):not(.active) {
      @apply !bg-atm-bg-brand border-b-atm-border-gray-medium border-r-atm-border-gray-medium;
