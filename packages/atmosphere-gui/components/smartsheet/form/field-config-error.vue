...
<template>
  <template v-if="mode === 'preview'">
    <div v-if="Object.keys(visibilityError ?? {}).length" class="flex mt-2">
      <AtTooltip :disabled="!firstErrorMsg" class="flex cursor-pointer" placement="bottom">
        <template #title>
        ...
        <div
          class="atm-field-config-error validation-error text-atm-content-red-dark bg-atm-bg-red-light rounded-lg inline-flex items-center gap-2 px-2 py-1"
        >
  ...
  <template v-else>
    <GeneralIcon
      ...
      icon="alertTriangle"
      class="ml-1 flex-none !text-atm-content-red-medium h-3.5 w-3.5"
    />
  </template>
</template>
