...
<template>
  ...
  >
    <AtTooltip v-if="tempFiles.length === 0" class="absolute top-3 right-3">
      <AtButton type="text" class="!border-0" :disabled="isLoading" size="xsmall" @click="closeModal">
    ...
    <div v-if="tempFiles.length > 0" class="flex w-full border-b-1 py-1 h-9.5 items-center justify-between top-0">
      <AtButton :disabled="isLoading" type="text" size="small" @click="clearFiles">
      ...
      <AtButton :disabled="isLoading" type="text" size="small" @click="open">
        ...
        >
          <div v-for="file in tempFiles" :key="file.name" class="flex gap-1.5 group max-w-34 pb-4 flex-col relative">
            <div
              ...
              style="height: 140px"
              class="flex items-center justify-center rounded-md bg-atm-bg-gray-light"
            >
    ...
    <AtFileUploadProvidersBottomBar v-if="tempFiles.length" />
  </div>
</template>
