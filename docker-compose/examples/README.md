...
|---------|-----------|-------|-------|----------|
| [quickstart-demo](quickstart-demo/) | Bundled | Bundled | None (port 8080) | Local eval / "show me Atmosphere" |
...
For the production-shaped examples (`managed-postgres` and friends), edit `docker-compose.yml`, `docker.env`, and `atmosphere/db.json` to replace placeholder values before running `docker compose up -d`.
...
After your stack is running, open Atmosphere, sign up as the first user, then activate a license at **Admin Panel → License**. PostgreSQL is required for license activation; every example here satisfies that.
