...
run_atmosphere() {
  run env ATMOSPHERE_SKIP_PREFLIGHT=1 bash "$(atmosphere_sh)" "$@"
}

@test "missing --pg fails" {
  run_atmosphere --domain=localhost --redis=bundled
  [ "$status" -eq 1 ]
  [[ "$output" == *"--pg=bundled or --pg=external is required"* ]]
}

@test "external PG without --pg-host fails" {
  run_atmosphere --domain=localhost --pg=external --redis=bundled
  [ "$status" -eq 1 ]
  [[ "$output" == *"--pg-host is required when --pg=external"* ]]
}

@test "external PG without --pg-user fails" {
  run_atmosphere --domain=localhost --pg=external --pg-host=db.example.com --redis=bundled
  [ "$status" -eq 1 ]
  [[ "$output" == *"--pg-user is required when --pg=external"* ]]
}

@test "external PG without --pg-password fails" {
  run_atmosphere --domain=localhost --pg=external --pg-host=db.example.com --pg-user=atmosphere --redis=bundled
  [ "$status" -eq 1 ]
  [[ "$output" == *"--pg-password is required when --pg=external"* ]]
}

@test "missing --redis fails" {
  run_atmosphere --domain=localhost --pg=bundled
  [ "$status" -eq 1 ]
  [[ "$output" == *"--redis=bundled or --redis=external is required"* ]]
}

@test "external Redis without --redis-url fails" {
  run_atmosphere --domain=localhost --pg=bundled --redis=external
  [ "$status" -eq 1 ]
  [[ "$output" == *"--redis-url is required when --redis=external"* ]]
}

@test "unknown flag fails" {
  run_atmosphere --domain=localhost --pg=bundled --redis=bundled --bogus
  [ "$status" -eq 1 ]
  [[ "$output" == *"Unknown flag"* ]]
}

@test "production domain without --acme-email fails" {
  run_atmosphere --domain=demo.example.com --pg=bundled --redis=bundled
  [ "$status" -eq 1 ]
  [[ "$output" == *"--acme-email is required"* ]]
}

@test "external PG with a CA path that does not exist fails" {
  run_atmosphere --domain=localhost --pg=external --pg-host=db.example.com --pg-user=atmosphere --pg-password=secretpass --pg-ssl=/no/such/ca.pem --redis=bundled
  [ "$status" -eq 1 ]
  [[ "$output" == *"CA file not found"* ]]
}

@test "misspelled --pg-ssl value fails instead of writing an empty CA" {
  run_atmosphere --domain=localhost --pg=external --pg-host=db.example.com --pg-user=atmosphere --pg-password=secretpass --pg-ssl=mananged --redis=bundled
  [ "$status" -eq 1 ]
  [[ "$output" == *"CA file not found"* ]]
}

@test "removed subcommands point to the new workflow" {
  run_atmosphere upgrade
  [ "$status" -eq 1 ]
