...
@test "external PG (managed SSL) matches the managed-postgres example" {
  ...
  assert_same_structure "$GEN_DIR/atmosphere/db.json" "$(examples_dir)/managed-postgres/atmosphere/db.json"
}
...
@test "external PG (custom CA) matches the postgres-private-ca example" {
  ...
  assert_same_structure "$GEN_DIR/atmosphere/db.json" "$(examples_dir)/postgres-private-ca/atmosphere/db.json"
}
...
@test "external PG + Redis matches the external-postgres-and-redis example" {
  ...
  assert_same_structure "$GEN_DIR/atmosphere/db.json" "$(examples_dir)/external-postgres-and-redis/atmosphere/db.json"
}
