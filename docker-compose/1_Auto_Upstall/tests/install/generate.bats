#!/usr/bin/env bats
#
# Verifies atmosphere.sh generates the right artifacts for each install scenario.
...
@test "production-ssl: real domain compose matches golden (traefik + Let's Encrypt)" {
  generate --domain=demo.example.com --acme-email=ssl@atmosphere.dev --pg=bundled --redis=bundled
...
@test "external-pg-managed: external PG (managed SSL) compose matches golden" {
  generate --domain=localhost --pg=external --pg-host=db.example.com --pg-user=atmosphere --pg-password=secretpass --pg-ssl=managed --redis=bundled
  assert_golden external-pg-managed
}

@test "external-pg-none: external PG (no SSL) compose matches golden" {
  generate --domain=localhost --pg=external --pg-host=db.example.com --pg-user=atmosphere --pg-password=secretpass --pg-ssl=none --redis=bundled
  assert_golden external-pg-none
}

@test "external-pg-custom: external PG (custom CA) compose matches golden" {
  generate --domain=localhost --pg=external --pg-host=db.example.com --pg-user=atmosphere --pg-password=secretpass --pg-ssl="$(fake_ca)" --redis=bundled
  assert_golden external-pg-custom
}

@test "external-pg-and-redis: fully external compose matches golden" {
  generate --domain=localhost --pg=external --pg-host=db.example.com --pg-user=atmosphere --pg-password=secretpass --pg-ssl=managed --redis=external --redis-url=redis://redis.example.com:6379
...
@test "bundled PG: db.json points at the bundled db service, no SSL block" {
  generate --domain=localhost --pg=bundled --redis=bundled
  grep -q '"client": "pg"' "$GEN_DIR/atmosphere/db.json"
  grep -q '"host": "db"' "$GEN_DIR/atmosphere/db.json"
  grep -q '"port": "5432"' "$GEN_DIR/atmosphere/db.json"
  grep -q '"database": "atmosphere"' "$GEN_DIR/atmosphere/db.json"
  ! grep -q '"ssl"' "$GEN_DIR/atmosphere/db.json"
}

@test "external PG (no SSL): db.json targets the external host with no SSL block" {
  generate --domain=localhost --pg=external --pg-host=db.example.com --pg-user=atmosphere --pg-password=secretpass --pg-ssl=none --redis=bundled
  grep -q '"host": "db.example.com"' "$GEN_DIR/atmosphere/db.json"
  ! grep -q '"ssl"' "$GEN_DIR/atmosphere/db.json"
}

@test "external PG (custom CA): db.json embeds the CA certificate" {
  generate --domain=localhost --pg=external --pg-host=db.example.com --pg-user=atmosphere --pg-password=secretpass --pg-ssl="$(fake_ca)" --redis=bundled
  grep -q '"rejectUnauthorized": true' "$GEN_DIR/atmosphere/db.json"
  grep -q 'BEGIN CERTIFICATE' "$GEN_DIR/atmosphere/db.json"
}
...
@test "bundled PG: a strong password is generated and shared by compose + db.json" {
  ...
  pw_json=$(grep '"password"' "$GEN_DIR/atmosphere/db.json" | sed -E 's/.*"password": "([^"]*)".*/\1/')
  [ "${#pw_env}" -ge 24 ]
  [[ "$pw_env" =~ ^[A-Za-z0-9]+$ ]]
  [ "$pw_env" = "$pw_json" ]
}
...
@test "docker.env carries the fixed settings block" {
  generate --domain=localhost --pg=bundled --redis=bundled
  grep -q '^ATMOSPHERE_DB_JSON_FILE=/usr/app/data/db.json$' "$GEN_DIR/docker.env"
  grep -q '^ATMOSPHERE_SECURE_ATTACHMENTS=true$' "$GEN_DIR/docker.env"
  grep -q '^ATMOSPHERE_DISABLE_MUX=true$' "$GEN_DIR/docker.env"
}

@test "docker.env: bundled Redis URL" {
  generate --domain=localhost --pg=bundled --redis=bundled
  grep -q '^ATMOSPHERE_REDIS_URL=redis://redis:6379$' "$GEN_DIR/docker.env"
}

@test "docker.env: external Redis URL is honoured" {
  generate --domain=localhost --pg=bundled --redis=external --redis-url=redis://localhost:6379
  grep -q '^ATMOSPHERE_REDIS_URL=redis://localhost:6379$' "$GEN_DIR/docker.env"
}

@test "docker.env: ATMOSPHERE_SITE_URL is the localhost URL in local mode" {
  generate --domain=localhost --pg=bundled --redis=bundled
  grep -q '^ATMOSPHERE_SITE_URL=http://localhost:8080$' "$GEN_DIR/docker.env"
}

@test "docker.env: ATMOSPHERE_SITE_URL is http://<ip> in production-ip mode" {
  generate --domain=1.2.3.4 --pg=bundled --redis=bundled
  grep -q '^ATMOSPHERE_SITE_URL=http://1.2.3.4$' "$GEN_DIR/docker.env"
}

@test "docker.env: ATMOSPHERE_SITE_URL is https://<domain> in production mode" {
  generate --domain=demo.example.com --acme-email=ssl@atmosphere.dev --pg=bundled --redis=bundled
  grep -q '^ATMOSPHERE_SITE_URL=https://demo.example.com$' "$GEN_DIR/docker.env"
}
...
@test "update.sh and .gitignore are generated" {
  ...
  grep -q '^atmosphere/db.json$' "$GEN_DIR/.gitignore"
}
